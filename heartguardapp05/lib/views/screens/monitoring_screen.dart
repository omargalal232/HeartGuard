        ...
        if (snapshot.exists) {
          try {
            ...
            if (lastUpdateTime == null ||
                timestamp.difference(lastUpdateTime!).inMilliseconds > 2) {
              _logger.i(_tag,
                  'New ECG reading: $value at ${timestamp.toIso8601String()}');
  ...
  Future<void> _saveHeartRateData(double rawValue, DateTime timestamp) async {
    ...
    try {
      ...
      await _firestore.collection('heartRateData').add({
        ...
        'abnormalityType': rawValue < 60
            ? 'low_heart_rate'
            : (rawValue > 100 ? 'high_heart_rate' : null),
      });
      ...
      if ((rawValue < 60 || rawValue > 100) &&
        ...
        final abnormalityType =
            rawValue < 60 ? 'low_heart_rate' : 'high_heart_rate';

        _logger.w(_tag,
            'Abnormal heart rate detected: $rawValue BPM - Sending FCM notification');
      ...
      _logger.e(
          _tag, 'Error saving heart rate data and sending notification', e);
    }
  }
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: const Text('Normal heart rate range: 60-100 BPM\n\n'
